...
训练集\\(\{(x_{1},y_{1}),(x_{2},y_{2}),\cdots,(x_{m},y_{m})\\)由样例(example)\\(\(x_{i},y_{i}\)\\)组成，其中\\(y_{i}\in\mathcal{Y}\\),\\(\mathcal{Y}\\)是所有标记(label)的集合，亦称“标记空间(label space)”或“输出空间”。一般地，预测任务是希望通过对训练集\\(\{(x_{1},y_{1}),(x_{2},y_{2}),\cdots,(x_{m},y_{m})\\)进行学习，建立一个输入空间\\(\mathcal{X}\\)到输出空间\\(\mathcal{Y}\\)的映射\\(f:\match{X}\mapsto\mathcal{Y}\\)。
