...
对于二分类问题可将样例根据真实类别与学习器预测类别的组合划分为：
* 真正例(true positive, TP)：真实类别和预测类别都为正类
* 假正例(false positive, FP)：真实分类为负类，而预测的分类为正类
* 真反例(true negative, TN)：真实分类和预测分类都为为负类
* 假反例(false negative, FN)：真实分类为正类，而预测的分类为负类


![分类结果混淆矩阵](/img/in-post/zzh-machine-learning/ch2/confusion_matrix.png)
<center>
分类结果混淆矩阵
</center>
...
查准率的公式可以理解为真正例在预测得到的正例中所占的比例  
查全率的公式可以理解为真正例在真实分类的正例中所占的比例  
